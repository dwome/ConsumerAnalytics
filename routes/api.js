        ...
        snapshot.forEach(function (snapshot2) {
            ...
            if (inputmail && obj.email == 1) {
                ...
                var mailOptions = {
                    ...
                    generateTextFromHTML: true,
                    html: "Hallo " + obj.email_address + ", <br />"  + notification.email_text + '<br /> Schau dir unsere Seite an unter: https://goo.gl/Hgtdy6 <img src="https://storage.googleapis.com/lidl-smart.appspot.com/currentBundle.jpg" height="50%" width="50%"/>',
                };
                smtpTransport.sendMail(mailOptions, function (error, response) {
                    if (error) {
                        console.log(error);
        ...
        snapshot.forEach(function (snapshot2) {
            ...
            if (phonenumber && obj.sms == 1) {
                ...
                client.sendMessage({
                    to: phonenumber,
                    from: '+4915735984837',
                    body: notification.sms_text + " https://goo.gl/Hgtdy6"
                }, function(err, data) {
                    if (err) {
                         console.log(err);
                       return res.status(500).send("Failure");
...
router.post("/sendWhatsAppUpdate_subscribe", function(req, res) {
    ...
    var options = { method: 'POST',
        ...
        form:
            { api_key: '1709510af522e46ea619b11642f3c3a8_4552_b41a2200d6875bf6bda88332cb',
                usernumber: new_phonenumber,
                content: 'Vielen Dank, du hast erfolgreich deine Einstellungen geändert und dich am Lidl-Smart-Shopping Newsletter angemeldet',
                msg_type: 'text' } };
